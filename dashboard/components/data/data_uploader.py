...
from dashboard.constants import DATA_CONFIG, FEATURES
from dashboard.components.data.validation import validate_file_upload, detect_encoding
    ...
    with st.expander("Upload your data", expanded=True):
        ...
        try:
            ...
            if file_extension == 'csv':
                df = pd.read_csv(uploaded_file, encoding=encoding, nrows=DATA_CONFIG["preview_rows"])
            ...
            # Show preview info
            st.success(f"Successfully loaded {uploaded_file.name}: {len(df)} rows × {df.shape[1]} columns")
