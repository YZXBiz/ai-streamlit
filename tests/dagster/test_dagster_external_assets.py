...
class TestExternalFeatureEngineeringAssets:
        ...
        context = build_asset_context(
            resources={
                "job_params": lambda: type(
                    "obj", (object,), {"min_features": 3, "correlation_threshold": 0.9}
                )
            }
        )

        # Call the asset function
        result = external_filtered_features(
            context, external_fe_raw_data=mock_features_with_missing
        )
    ...
    def test_imputed_features(self, mock_features_with_missing) -> None:
        ...
        # Call the asset function
        result = external_imputed_features(
            context, external_filtered_features=mock_features_with_missing
        )
    ...
    def test_outlier_removed_features(self, mock_features_with_missing) -> None:
        ...
        context = build_asset_context(
            resources={
                "job_params": lambda: type(
                    "obj", (object,), {"outlier_method": "z-score", "z_threshold": 3.0}
                )
            }
        )
    ...
    def test_dimensionality_reduced_features(self, mock_features_with_missing) -> None:
        ...
        context = build_asset_context(
            resources={
                "job_params": lambda: type(
                    "obj", (object,), {"pca_components": 2, "random_state": 42}
                )
            }
        )
...
class TestExternalClusteringAssets:
    ...
    def test_optimal_cluster_counts(self, mock_features_with_missing) -> None:
        ...
        context = build_asset_context(
            resources={
                "job_params": lambda: type(
                    "obj", (object,), {"min_clusters": 2, "max_clusters": 4, "random_state": 42}
                )
            }
        )
    ...
    def test_save_clustering_models(self, mock_features_with_missing) -> None:
        ...
        def mock_writer(model_dict, *args, **kwargs):
...
def test_external_pipeline_integration() -> None:
    ...
    try:
        ...
        prep_result = materialize_to_memory(
            external_preprocessing_job.asset_selection, resources=resources
        )
        ...
        prep_keys = {key.to_user_string() for key in prep_result.asset_values.keys()}
    ...
    try:
        ...
        fe_assets = [
            asset
            for asset in external_ml_job.asset_selection
            if any(
                x in asset.key.to_user_string()
                for x in [
                    "fe_raw_data",
                    "filtered",
                    "imputed",
        ...
        if fe_assets:
            fe_result = materialize_to_memory(fe_assets, resources=resources)
...
def test_external_asset_checks() -> None:
    ...
    def check_location_id_present(
        context, external_features_data: pd.DataFrame
    ) -> AssetCheckResult:
    ...
    def check_normalized_range(context, external_normalized_data: pd.DataFrame) -> AssetCheckResult:
        ...
        for col in numeric_cols:
            if (
                external_normalized_data[col].max() > 1.0
                or external_normalized_data[col].min() < -1.0
            ):
                all_in_range = False
