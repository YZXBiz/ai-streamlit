...
class TestInternalOptimalClusterCounts:
    ...
    def test_optimal_cluster_counts(self, mock_exp_class, mock_execution_context, sample_category_data):
        ...
        for category, data in result.items():
            ...
            assert data["optimal_clusters"] == 3  # Mock data has max silhouette at index 1 (3 clusters)
            assert "silhouette" in data["metrics"]
            assert "calinski_harabasz" in data["metrics"]
    
    @patch("clustering.pipeline.assets.clustering.internal_ml.model_training.ClusteringExperiment")
    def test_different_optimization_metric(self, mock_exp_class, mock_execution_context, sample_category_data):
        ...
        for category, data in result.items():
            assert data["optimal_clusters"] == 4  # Should be index 2 (4 clusters) for calinski_harabasz
...
class TestInternalTrainClusteringModels:
    ...
    @patch("clustering.pipeline.assets.clustering.internal_ml.model_training.ClusteringExperiment")
    def test_train_models(self, mock_exp_class, mock_execution_context, sample_category_data, sample_clustering_results):
