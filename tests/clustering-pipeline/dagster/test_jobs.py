...
class TestJobBasics:
    ...
    def test_asset_job(self, in_memory_dagster_instance: DagsterInstance) -> None:
        ...
        asset_job = define_asset_job(
            name="test_asset_job",
            selection=[AssetKey("input_asset"), AssetKey("output_asset")]
        )
...
class TestInternalPreprocessingJob:
    ...
    def test_internal_preprocessing_job_execution(
        self, 
        in_memory_dagster_instance: DagsterInstance
    ) -> None:
        ...
        assert any("internal_load_raw_data" in str(node.asset_key) 
                  for node in job_def.asset_selection_data)
        assert any("internal_preprocess_data" in str(node.asset_key)
                  for node in job_def.asset_selection_data)
