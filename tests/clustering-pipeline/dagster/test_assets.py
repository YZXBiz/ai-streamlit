...
class TestInternalAssets:
    ...
    def test_internal_preprocess_data(
        ...
        internal_raw_data: pd.DataFrame
    ) -> None:
    ...
    def test_internal_transform_data(
        ...
        # Execute the asset
        result = internal_transform_data(mock_asset_context, internal_preprocessed_data, external_preprocessed_data, config)
        
        # Validate result
        assert isinstance(result, pd.DataFrame)
        assert "CLUSTER" in result.columns  # Should add this column
        assert set(result["CLUSTER"].unique()) <= {0, 1, 2}  # Cluster values should be in expected range
...
class TestExternalAssets:
    ...
    def test_external_preprocess_data(
        self, 
        mock_asset_context: AssetExecutionContext, 
        external_raw_data: pd.DataFrame
    ) -> None:
...
class TestMergingAssets:
    ...
    def test_merge_clusters(
        ...
        # Execute the asset
        result = merge_clusters(mock_asset_context, internal_transformed_data, external_transformed_data, config)
        ...
        assert all(0 <= score <= 1 for score in result["match_score"])  # Match scores should be in [0,1]
...
class TestAssetMaterialization:
    ...
    def test_asset_materialization(self, in_memory_dagster_instance: DagsterInstance) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            @asset(
                ins={"input_data": AssetIn("test_input_asset")}
            )
