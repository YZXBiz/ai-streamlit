...
from clustering.cli.commands import run_command, export_command, status_command
from clustering.pipeline.definitions import full_pipeline_job, internal_preprocessing_job, external_preprocessing_job, merging_job
from clustering.shared.schemas import SalesSchema, NSMappingSchema
...
class TestFullPipelineE2E:
    ...
    @pytest.mark.slow
    @pytest.mark.e2e
    def test_full_pipeline_execution(self, test_data_dir: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        result = runner.invoke(
            run_command, 
            ["full_pipeline_job", "--config", str(config_path)]
        )
        ...
        assert "sku_nbr" in results.columns.str.lower().tolist() or "product_id" in results.columns.str.lower().tolist()
    ...
    def test_pipeline_with_different_algorithms(
        ...
        output_lines = result.output.strip().split("\n")
        job_id = next(
            (line.split(":")[-1].strip() for line in output_lines if "job_id" in line.lower()),
            None
        )
    ...
    def test_pipeline_with_different_cluster_counts(
        ...
        cluster_col = next(col for col in results.columns if "cluster" in col.lower())
        assert results[cluster_col].nunique() <= n_clusters  # Can be less if some clusters are empty
    ...
    def test_pipeline_with_empty_data(
        ...
        result = runner.invoke(
            run_command, 
            ["full_pipeline_job", "--config", str(empty_config_path)]
        )
    ...
    def test_pipeline_with_invalid_config(
        ...
        result = runner.invoke(
            run_command, 
            ["full_pipeline_job", "--config", str(invalid_config_path)]
        )
...
class TestFullPipelineE2EDagster:
    ...
    def test_full_pipeline_dagster_execution(
        ...
        run_config = {
            "ops": {
                "internal_load_raw_data": {
                    "config": {
                        "data_path": config["job"]["data_paths"]["internal_sales"]
                    }
                },
                "external_load_raw_data": {
                    "config": {
                        "data_path": config["job"]["data_paths"]["external_mapping"]
                    }
                },
                ...
                "merge_clusters": {
                    "config": {
                        ...
                        "output_path": os.path.join(config["job"]["data_paths"]["output_dir"], "merged_results.csv")
                    }
                }
            },
            "resources": {
                "io_manager": {
                    "config": {
                        "base_dir": str(test_data_dir / "dagster_output")
                    }
                }
            }
        }
        ...
        executed_steps = [event.step_key for event in result.all_node_events if event.is_step_success]
        ...
        # Check output file
        merged_output = os.path.join(config["job"]["data_paths"]["output_dir"], "merged_results.csv")
    ...
    def test_individual_pipeline_components(
        ...
        internal_result = execute_job(
            internal_preprocessing_job, 
            run_config=internal_config
        )
    ...
    def test_pipeline_performance(
        ...
        for step, timing in step_timing.items():
            if "duration" in timing:
                ...
                assert step_ratio < 0.7, f"Step {step} took {step_ratio:.1%} of total execution time" 