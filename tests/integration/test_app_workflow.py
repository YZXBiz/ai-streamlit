...
def test_full_app_workflow(
    mock_streamlit, mock_session_state, mock_uploaded_file, mock_agent, sample_dataframe
):
    ...
    with (
        patch(
            "app.components.uploader_components.load_dataframe",
            return_value=(sample_dataframe, None),
        ),
        patch(
            "app.components.uploader_components.initialize_agent", return_value=(mock_agent, None)
        ),
    ...
    with patch(
        "app.components.chat_components.process_response",
        return_value=("text", "The average is 3.0"),
    ):
        # Run main for chat
        main()
        ...
        assert (
            len(mock_session_state["chat_history"]) == 3
        )  # Welcome message + user question + response
        assert mock_session_state["chat_history"][1]["role"] == "user"
        assert (
            mock_session_state["chat_history"][1]["content"] == "What is the average of column A?"
        )
