...
class TestAssetChecks:
    ...
    def test_column_existence_check(self, mock_sales_data) -> None:
        ...
        def check_required_columns(
            context, internal_raw_sales_data: pd.DataFrame
        ) -> AssetCheckResult:
    ...
    def test_value_range_check(self, mock_features_data) -> None:
        ...
        def check_normalized_values(
            context, internal_normalized_data: pd.DataFrame
        ) -> AssetCheckResult:
        ...
        # Create context
        context = build_asset_context(asset_key=AssetKey("internal_normalized_data"))
    ...
    def test_data_freshness_check(self) -> None:
        ...
        from dagster import MetadataValue, asset_check
        ...
        def check_data_freshness(context: AssetCheckExecutionContext) -> AssetCheckResult:
            ...
            if context.asset_events:
                ...
                if isinstance(last_materialized, MetadataValue):
                    ...
                    return AssetCheckResult(
                        passed=False, metadata={"error": "No materialization time available"}
                    )
            ...
            return AssetCheckResult(
                passed=False, metadata={"error": "No materialization events found"}
            )
    ...
    def test_cluster_quality_check(self, mock_clustered_data) -> None:
        ...
        def check_cluster_quality(
            context, internal_assign_clusters: pd.DataFrame
        ) -> AssetCheckResult:
            ...
            for cluster_id in internal_assign_clusters["cluster_id"].unique():
                cluster_data = internal_assign_clusters[
                    internal_assign_clusters["cluster_id"] == cluster_id
                ]
            ...
            clusters_above_threshold = [
                cid
                for cid, stats in cluster_distances.items()
        ...
        # Create context
        context = build_asset_context(asset_key=AssetKey("internal_assign_clusters"))

        # Test with good data - store result in _ since we're not using it
        _ = check_cluster_quality(context, mock_clustered_data)
...
def test_message_check():
    ...
    @check_message("Internal features have null values in columns: {null_columns}")
...
def test_defining_asset_checks_in_defs():
    ...
    from dagster import Definitions, asset, asset_check
    ...
    try:
        ...
        check_results = [
            event for event in result.asset_check_events if event.asset_key == test_data.key
        ]
...
def test_run_subset_of_checks():
    ...
    from dagster import AssetChecksDefinition, Definitions, asset, asset_check
    ...
    def check_row_count(context, sample_data: pd.DataFrame) -> AssetCheckResult:
        ...
        return AssetCheckResult(
            passed=row_count >= 3, metadata={"row_count": row_count, "min_required": 3}
        )
    ...
    def check_id_column(context, sample_data: pd.DataFrame) -> AssetCheckResult:
        ...
        return AssetCheckResult(
            passed=has_id and unique_ids, metadata={"has_id": has_id, "unique_ids": unique_ids}
        )
    ...
    def check_values_positive(context, sample_data: pd.DataFrame) -> AssetCheckResult:
        ...
        if "value" not in sample_data.columns:
            return AssetCheckResult(passed=False, metadata={"error": "value column not found"})

        min_value = float(sample_data["value"].min())
        all_positive = min_value > 0

        return AssetCheckResult(
            passed=all_positive, metadata={"min_value": min_value, "all_positive": all_positive}
        )
    ...
    try:
        ...
        # Get the asset check results
        check_results = [
            event for event in result.asset_check_events if event.asset_key == sample_data.key
        ]
