    ...
    def test_asset_selection(self) -> None:
        ...
        for job_name, job in jobs.items():
            ...
            if job_name == "full_pipeline_job":
                assert len(asset_selection) > sum(
                    len(j.asset_selection)
                    for j_name, j in jobs.items()
                    if j_name != "full_pipeline_job"
                )
...
def test_definitions_integration() -> None:
    ...
    try:
        ...
        with DagsterInstance.ephemeral():
            ...
            @asset
            def test_asset() -> None:
                """Test asset for Dagster definition test."""
                pass

            # Create a job but don't assign it to avoid the F841 error
            define_asset_job("test_job", selection=[test_asset])
