...
class TestInternalPreprocessingAssets:
    ...
    def test_normalized_sales_data(self, mock_sales_data) -> None:
        ...
        sales_with_categories["NEED_STATE"] = ["State A", "State B", "State A", "State C", "State B"]
        
        result = internal_normalized_sales_data(context, internal_sales_with_categories=sales_with_categories)
    ...
    def test_optimal_cluster_counts(self, mock_features_data) -> None:
        ...
        try:
            result = internal_optimal_cluster_counts(
                context, 
                internal_dimensionality_reduced_features=mock_features_data.drop("entity_id", axis=1).fillna(0)
            )
...
def test_preprocessing_pipeline() -> None:
    ...
    try:
        ...
        result = materialize_to_memory(
            internal_preprocessing_job.asset_selection,
            resources=resources
        )
...
def test_asset_check_example() -> None:
    ...
    def check_sales_data_has_required_columns(context: AssetExecutionContext, internal_raw_sales_data: pd.DataFrame) -> AssetCheckResult:
