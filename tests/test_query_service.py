...
class TestQueryService(unittest.TestCase):
    ...
    def test_successful_query_execution(self, mock_data_service, mock_agent):
        ...
        # Process a query
        result = self.query_service.process_query("Show me all products", self.test_data)
    ...
    def test_query_execution_error(self, mock_data_service, mock_agent):
        ...
        # Mock the agent response
        mock_response = QueryResponse(
            sql="INVALID SQL QUERY", explanation="This will fail", visualization_type=None
        )
        mock_agent.return_value = mock_response
        ...
        # Process a query
        result = self.query_service.process_query("Run an invalid query", self.test_data)
    ...
    def test_agent_no_sql_generated(self, mock_data_service, mock_agent):
        ...
        mock_response = QueryResponse(
            sql="", explanation="Could not understand the query", visualization_type=None
        )
        mock_agent.return_value = mock_response
        ...
        result = self.query_service.process_query(
            "A confusing query that can't be processed", self.test_data
        )
    ...
    def test_exception_handling(self, mock_agent):
        ...
        # Process a query
        result = self.query_service.process_query("Query that causes an exception", self.test_data)
    ...
    def test_visualization_suggestion(self):
        ...
        result = self.query_service._suggest_visualization(
            ...
            pd.DataFrame(
                {"date": ["2023-01-01", "2023-01-02", "2023-01-03"], "price": [10, 15, 12]}
            ),
        )
        ...
        result = self.query_service._suggest_visualization(
            ...
            pd.DataFrame({"product": ["Apple", "Banana", "Orange"], "quantity": [100, 150, 80]}),
        )
        ...
        result = self.query_service._suggest_visualization(
            ...
            pd.DataFrame({"price": [1.2, 0.5, 0.8, 2.5, 1.5], "quantity": [100, 150, 80, 60, 70]}),
        )
