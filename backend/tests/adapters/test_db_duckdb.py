...
import pandas as pd
import pytest
from app.adapters.db_duckdb import DuckDBDataSource
...
class TestDuckDBDataSource:
    ...
    def test_get_schema(self, temp_csv_file):
        ...
        assert {"name": "id", "type": "INTEGER"} in columns
        assert {"name": "name", "type": "VARCHAR"} in columns
        assert {"name": "value", "type": "DOUBLE"} in columns         assert {"name": "value", "type": "DOUBLE"} in columns 