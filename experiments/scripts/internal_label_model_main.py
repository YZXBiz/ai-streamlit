...
def run_multi_cluster_pipeline(
    ...
    if directory is None:
        raise ValueError(
            "No directory specified. Please provide a valid path to the clustered data."
        )
    ...
    for key in clustered_data.keys():
        ...
        shap_table_long["renamed_feature"] = (
            shap_table_long["feature"].map(rename_dict).fillna(shap_table_long["feature"])
        )
        ...
        grouped["normalized_mean_abs_shap"] = grouped["mean_abs_shap"] / (
            total_shap if total_shap != 0 else 1
        )
