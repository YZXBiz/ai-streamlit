...
def apply_pca(data, pca_n_features_threshold, max_n_pca_features, 
              cumulative_variance_target=0.95, random_state=9999):
    """
    ...
    if n_features > pca_n_features_threshold:
        ...
        logging.info(
            "PCA not applied because number of features <= pca_n_features_threshold."
        )
        return data, None


def run_kmeans_and_evaluate(data, 
    ...
    C_min, C_max = results_df["calinski_harabasz_score"].min(), results_df["calinski_harabasz_score"].max()
    D_min, D_max = results_df["davies_bouldin_score"].min(), results_df["davies_bouldin_score"].max()
