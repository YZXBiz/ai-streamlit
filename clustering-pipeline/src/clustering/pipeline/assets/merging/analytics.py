...
def cluster_labeling_analytics(
    ...
    output_directory = os.path.join(output_path, f"Classics_Output_Run_{current_datetime_str}")
    ...
    try:
        ...
        context.log.info(
            f"Loaded merged cluster assignments with {len(merged_clusters_dict)} categories"
        )
    ...
    try:
        ...
        context.log.info(
            f"Loaded need states mapping data with {df_need_states_mapping.shape[0]} rows"
        )
    ...
    try:
        ...
        missing_columns = [
            col for col in needed_columns if col not in df_need_states_mapping.columns
        ]
        if missing_columns:
    ...
    try:
        ...
        context.log.info(
            f"Merged need states data: {rows_before} rows before, {rows_after} rows after merge"
        )
    except Exception as e:
        ...
        try:
            ...
            if missing_cols:
                ...
                if len(missing_cols) > len(id_cols) / 2:
                    context.log.warning(
                        f"Skipping category {category} due to too many missing columns"
                    )
                    continue
            ...
            if not available_id_cols:
                context.log.warning(
                    f"No ID columns available in clustering output for category {category}. Skipping."
                )
                continue

            clustering_output = clustering_output_temp[available_id_cols].drop_duplicates().copy()
            context.log.info(
                f"Prepared clustering output with columns: {clustering_output.columns.tolist()}"
            )
            ...
            if df_grouped_cat.empty:
                ...
                similar_categories = [
                    c for c in available_categories if c.lower() == fixed_category.lower()
                ]
            ...
            missing_required = [
                col for col in required_cluster_cols if col not in df_final_cat.columns
            ]
            if missing_required:
            ...
            excel_filename = (
                f"classics_output_{safe_category_for_filename}_{current_datetime_str}.xlsx"
            )
    ...
    try:
        ...
        if not validation_result["passed"]:
            ...
            if fixed_validation_result["passed"]:
                ...
                context.log.warning(
                    f"Schema validation still failed after fixing: {fixed_validation_result}"
                )
        else:
        ...
        store_count = (
            combined_df["store_nbr"].nunique() if "store_nbr" in combined_df.columns else 0
        )
        ...
        context.log.info(
            f"Data successfully written to {SF_DATABASE}.{SF_SCHEMA}.{OUTPUT_TABLE_NAME}"
        )
        ...
        context.log.info(
            f"Archive copy with timestamp {timestamp} written to {SF_DATABASE}.{SF_SCHEMA}.{ARCHIVE_TABLE_NAME}"
        )
