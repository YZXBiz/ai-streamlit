"""Visualization utilities for the clustering dashboard.

This module provides helper functions for visualizations in the Streamlit dashboard.
"""
...
def get_color_scale(
    scale_type: Literal["sequential", "diverging", "qualitative"] = "sequential",
) -> list:
...
def format_file_size(size_bytes: int) -> str:
    ...
    return f"{size_bytes_float:.2f} TB"
...
def create_figure_with_dropdown(
    df: pd.DataFrame,
    columns: list[str],
    color_col: str | None = None,
    title: str = "Interactive Plot with Dropdown",
) -> go.Figure:
    ...
    for i, column in enumerate(columns):
        ...
        buttons.append(
            dict(
                method="update",
                args=[{"visible": visible}, {"title": f"{title}: {column}"}],
                label=column,
            )
        )
        ...
        if pd.api.types.is_numeric_dtype(df[col]):
            ...
            if df[col].nunique() <= 10:
                ...
                filters[col] = st.sidebar.multiselect(
                    f"Select {col}", options=options, default=options
                )
            else:
            ...
            options = sorted(df[col].unique())
            filters[col] = st.sidebar.multiselect(f"Select {col}", options=options, default=options)
...
def plot_correlation_matrix(
    ...
    # Update layout
    fig.update_layout(
        height=700 if len(columns) > 10 else 500, width=700 if len(columns) > 10 else 500
    )
...
def create_time_series_plot(
    ...
    if group_col:
        fig = px.line(
            ...
            color=group_col,
            title=title,
            labels={date_col: "Date", value_col: "Value", group_col: "Group"},
        )
    else:
        fig = px.line(
            df, x=date_col, y=value_col, title=title, labels={date_col: "Date", value_col: "Value"}
        )

    # Add range slider
    fig.update_layout(xaxis=dict(rangeslider=dict(visible=True), type="date"), height=500)
    ...
    with st.expander("Time Aggregation"):
        ...
        agg_method = st.selectbox(
            "Aggregation Method", options=["mean", "sum", "min", "max", "count"], index=0
        )

        agg_period = st.selectbox(
            "Aggregation Period", options=["Day", "Week", "Month", "Quarter", "Year"], index=2
        )
        ...
        if group_col:
            ...
            agg_df = (
                df_copy.groupby(group_col)
                .resample(period_map[agg_period])[value_col]
                .agg(agg_method)
                .reset_index()
            )
            ...
            agg_fig = px.line(
                ...
                y=value_col,
                title=f"{title} ({agg_method} by {agg_period})",
                labels={date_col: "Date", value_col: f"{agg_method.capitalize()} of {value_col}"},
            )
