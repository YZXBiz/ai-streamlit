    ...
    for col, expected_type in expected_schema.items():
        if col in df.columns:
            ...
            if expected_type == str:
                if not pd.api.types.is_object_dtype(df[col]) and not pd.api.types.is_string_dtype(
                    df[col]
                ):
                    type_mismatches.append((col, str(df[col].dtype), str(expected_type)))
            # Special case for integers
            elif expected_type == int:
                if not pd.api.types.is_integer_dtype(df[col]) and not pd.api.types.is_float_dtype(
                    df[col]
                ):
                    type_mismatches.append((col, str(df[col].dtype), str(expected_type)))
            # Special case for floats
            elif expected_type == float:
                if not pd.api.types.is_float_dtype(df[col]) and not pd.api.types.is_integer_dtype(
                    df[col]
                ):
                    type_mismatches.append((col, str(df[col].dtype), str(expected_type)))
        ...
        if col in df_copy.columns:
            if col_type == str:
                ...
                if not pd.api.types.is_object_dtype(
                    df_copy[col]
                ) and not pd.api.types.is_string_dtype(df_copy[col]):
